...
public static partial class Tasks
{
    ...
    public static IEnumerable<Emp> Task4()
    {
        ...
        return Emps.Where(x => x.Salary == Emps.Max(e=>e.Salary));
    }
    ...
    {
        return Emps.Count(x => x.Job == "Backend programmer") > 0;
    }
    ...
    {
        return Emps.OrderByDescending(x=>x.HireDate).First(x => x.Job == "Frontend programmer");
    }
    ...
    {
        return Emps.Join(Depts, emp => emp.Deptno, dept => dept.Deptno, ((emp, dept) => new { dept.Dname, emp.Empno }))
            .GroupBy(dept => dept.Dname).Select(value => new { name = value.Key, numOfEmployees = value.Count() })
            .Where(value => value.numOfEmployees > 1).ToList();
    }
    ...
    public static IEnumerable<Dept> Task14()
    {
        return Depts
            .Where(dept =>
                Emps.Count(x => x.Deptno == dept.Deptno) == 5 || Emps.Count(x => x.Deptno == dept.Deptno) == 0)
